...
class ensembleNN(defaultSettings):
    ...
    def trainEnsemble(self,dataTable,X,y):
        dataTable = dataTable[X + [y]].dropna().copy()
        if self.scaleY:
            dataTable[y] = self.y_norm(dataTable[y].values)
        self.modelEnsemble = {}
...
ensembleNN(hiddenLayerShape=(10)).trainEnsemble(dataTable,X=['TA_1_1_1','SW_IN_1_1_1'],y='TS_1_1_1')
